    ...
    },
    {
      ...
      cellRenderer: function(params) {
        ...
        _.forEach(params.data, (value, key) => {
          ...
          logString +=
            `<span class="cname-hide-${string2Hex(key)}">${colSep}${key}=` +
            JSON.stringify(JSON.unflatten(value), null, 2) +
            `</span>`;
          counter++;
        });
        return logString;
...
function renderPanelLogsGrid(columnOrder, hits, panelId, logLinesViewType) {
  ...
  // for panels on the dashboard page
    ...
    panelGridDiv = document.querySelector(
      `#panel${panelId} #panelLogResultsGrid`
    );
  ...
  panelLogsColumnDefs = _.chain(panelLogsColumnDefs)
    .concat(cols)
    .uniqBy("field")
    .value();
...
function panelLogOptionSingleHandler(panelGridOptions, panelLogsColumnDefs) {
  ...
  panelGridOptions.columnApi.autoSizeColumn(
    panelGridOptions.columnApi.getColumn("logs"),
    false
  );
}

function panelLogOptionMultiHandler(
  panelGridOptions,
  panelLogsColumnDefs,
  panelLogsRowData
) {
...
function renderPanelAggsGrid(columnOrder, hits, panelId) {
  ...
  colDefs = columnOrder.map((colName, index) => {
    ...
    let maxWidth = Math.max(
      displayTextWidth(colName, "italic 19pt  DINpro "),
      200
    ); //200 is approx width of 1trillion number
    return {
      field: title,
      headerName: title,
      resizable: resize,
  ...
  aggsColumnDefs = _.chain(aggsColumnDefs)
    .concat(colDefs)
    .uniqBy("field")
    .value();
  ...
  $.each(hits.measure, function (key, resMap) {
    ...
    columnOrder.map((colName, index) => {
      ...
      if (
        ...
        if (
          resMap.MeasureVal[colName] === undefined ||
