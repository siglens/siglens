...
$(function () {
    ...
    activate: function (_event, _ui) {
      ...
      if (currentTab == 0) {
        ...
        if (!isQueryBuilderSearch) {
          ...
          $(".tags-list").empty();
          [firstBoxSet, secondBoxSet, thirdBoxSet] = [
            new Set(),
            new Set(),
            new Set(),
          ];
          $(
            "#aggregations, #aggregate-attribute-text, #search-filter-text"
          ).show();
        }
  ...
  function checkFirstBox(curSelect) {
    ...
    if (
      ...
      num++;
    if (
      ($("#symbol").val() == null || $("#symbol").val().trim() == "") &&
      curSelect != 1
    )
      num++;
    if (
      ($("#value-first").val() == null || $("#value-first").val().trim() == "") &&
      curSelect != 2
    )
      num++;
    if (num != 0) {
  ...
  function getSearchText() {
    ...
    if (
      filterValue ===
      "Searches with a Search Criteria must have an Aggregate Attribute"
    ) {
  ...
  $("#column-second")
    .autocomplete({
      ...
      select: async function (event, ui) {
        ...
        $("#value-second").val("");
        if (columnsNames.length === 0 || numericColumns.length == 0)
          await getColumns();
  ...
  async function getColumns() {
    ...
    if (res) {
      columnsNames = res.allColumns.filter(
        (column) => column !== "_index" && column !== "timestamp"
      ); // remove '_index' and 'timestamp' column from query builder
