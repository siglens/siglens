...
$(document).ready(function () {
  ...
  panelContainerWidthGlobal = isActive
    ? panelContainer.offsetWidth - 97
    : panelContainer.offsetWidth - 215;
...
function recalculatePanelWidths() {
  localPanels.map((localPanel) => {
    localPanel.gridpos.w =
      localPanel.gridpos.wPercent * panelContainerWidthGlobal;
  });
}
...
function updateDashboard() {
  ...
  fetch("/api/dashboards/update", {
    ...
    .then((res) => {
      if (res.status == 200) {
        displayDashboardName();
        console.log("result after updating the record", res);
...
function handleDescriptionTooltip(panelId, description) {
  ...
  const panelDescIcon = $(
    `#panel${panelId} .panel-info-corner #panel-desc-info`
  );
...
function resetPanelLocationsHorizontally() {
  ...
  let indices = [];
  for (let i = 0; i < temp.length; i++) indices.push(temp[i][1]);

  for (let i = 0; i < indices.length; i++) {
    let hRight = localPanels[indices[i]].gridpos.h;
    let wRight =
      localPanels[indices[i]].gridpos.wPercent * panelContainerWidthGlobal;
    ...
    for (let j = 0; j < i; j++) {
      ...
      if (
        (yLeft >= yRight && yLeft <= yRight + hRight) ||
        (yLeft + hLeft >= yRight && yLeft + hLeft <= yRight + hRight) ||
        (yLeft <= yRight && yLeft + hLeft >= yRight + hRight)
      ) {
    ...
    if (
      xmax + wRight <
      $("#panel-container")[0].offsetWidth +
        $("#panel-container")[0].offsetLeft -
        20
    ) {
...
function resetPanelLocationsVertically() {
  ...
  for (let i = 0; i < indices.length; i++) {
    ...
    for (let j = 0; j < i; j++) {
      ...
      if (
        (xTop >= xDown && xTop <= xDown + wDown) ||
        (xTop + wTop >= xDown && xTop + wTop <= xDown + wDown) ||
        (xTop <= xDown && xTop + wTop >= xDown + wDown)
      ) {
...
function renderDuplicatePanel(duplicatedPanelIndex) {
  let boundaryY =
    localPanels[duplicatedPanelIndex].gridpos.y +
    localPanels[duplicatedPanelIndex].gridpos.h;
  for (let i = 0; i < localPanels.length; i++) {
    if (localPanels[i].panelIndex == localPanels.length - 1) continue; // this is the newly created duplicate panel.
    if (localPanels[i].gridpos.y >= boundaryY)
      // if any panel starts after the ending Y-cordinate of the duplicated panel, then it should be shifted downwards
      localPanels[i].gridpos.y +=
        localPanels[duplicatedPanelIndex].gridpos.h + 20;
  }
  resetPanelLocationsVertically();
  resetPanelLocationsHorizontally();
  resetPanelContainerHeight();
  ...
  $(
    `#panel${localPanels[localPanels.length - 1].panelId} .panel-header p`
  ).html(localPanels[duplicatedPanelIndex].name + "Copy");

  if (
    localPanel.chartType == "Data Table" ||
    localPanel.chartType == "loglines"
  ) {
    ...
    if (localPanel.queryRes)
      runPanelAggsQuery(
        localPanel.queryData,
        localPanel.panelId,
        localPanel.chartType,
        localPanel.dataType,
        localPanel.panelIndex,
        localPanel.queryRes
      );
    else
      runPanelAggsQuery(
    ...
    localPanel.chartType == "Pie Chart" ||
    localPanel.chartType == "Bar Chart"
  ) {
...
function updateTimeRangeForPanels() {
  localPanels.forEach((panel) => {
    ...
    if (panel.queryData) {
      if (
        (panel.chartType === "Line Chart" || panel.chartType === "number") &&
        panel.queryType === "metrics"
      ) {
        datePickerHandler(
          panel.queryData.start,
          panel.queryData.end,
          panel.queryData.start
        );
        ...
        datePickerHandler(
          panel.queryData.startEpoch,
          panel.queryData.endEpoch,
          panel.queryData.startEpoch
        );
...
function displayPanels() {
  ...
  localPanels.map((localPanel) => {
    ...
    let panel = $("<div>")
      .append(panelLayout)
      .addClass("panel")
      .attr("id", `panel${idpanel}`)
      .attr("panel-index", localPanel.panelIndex);
...
function showToast(msg) {
  let toast = `<div class="div-toast" id="save-db-modal">
        ${msg}
...
function handleResize(panelId) {
  $(`#panel${panelId}`).resizable({ containment: "parent" });
  $(`#panel${panelId}`).on("resizestop", function (event, ui) {
    ...
    localPanels[panelIndex].gridpos.wPercent =
      ui.size.width / panelContainerWidthGlobal;
...
function resizePanelFontSize(panelIndex, panelId) {
  if (panelIndex !== -1) {
    ...
    if (numFontSize > 170) numFontSize = 170;
    $(bigNumText).css("font-size", `${numFontSize}px`);

    panelWidth = parseFloat(localPanels[panelIndex].gridpos.w);

    if (
      bigNumText.width() + $(`#panel${panelId} .unit`).width() >=
      panelWidth
    ) {
      numFontSize -=
        (bigNumText.width() +
          $(`#panel${panelId} .unit`).width() -
          panelWidth) /
        3.5;
    }
    ...
    if (numFontSize < 50) numFontSize = 50;
    $(bigNumText).css("font-size", `${numFontSize}px`);
...
function handleDrag(panelId) {
  ...
  $(`#panel${panelId}`).on("dragstop", function (event, ui) {
    ...
    if (
      ui.position.left + $(this).width() <
      $("#panel-container")[0].offsetWidth + $("#panel-container")[0].offsetLeft
    ) {
...
function checkForAddigInTopRow() {
  ...
  if (rightBoundary <= panelContainerWidth * 0.49 + 10)
    return [true, rightBoundary, topmostY];
...
function addPanel(panelToDuplicate) {
  ...
  let panelElement = document.getElementById(`panel${idpanel}`);
  let panelHeight = panelToDuplicate
    ? panelToDuplicate.gridpos.h
    : panelElement.offsetHeight;
  let panelWidth = panelToDuplicate
    ? panelToDuplicate.gridpos.w
    : panelElement.offsetWidth;
  let panelTop = panelToDuplicate
    ? panelToDuplicate.gridpos.y + panelToDuplicate.gridpos.h + 20
    : marginTop + 20;
  let panelLeft = panelToDuplicate
    ? panelToDuplicate.gridpos.x
    : panelElement.offsetLeft;
  let panelWidthPercentage = panelWidth / panelContainerWidthGlobal;

  if (panelToDuplicate == undefined) {
    // means a new panel is being added
  ...
  if (panelToDuplicate) {
    ...
    if (panelToDuplicate.description) {
      handleDescriptionTooltip(
        panelToDuplicate.panelId,
        panelToDuplicate.description
      );
    }
  }
