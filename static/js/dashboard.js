...
function handlePanelDuplicate() {
  ...
  $(".panel-dupl-li").on("click", async function () {
    ...
    addDuplicatePanel(
      JSON.parse(JSON.stringify(localPanels[duplicatedPanelIndex]))
    );
...
function renderDuplicatePanel(duplicatedPanelIndex) {
  ...
  if (localPanel.description || localPanel.queryData.searchText) {
    handleDescriptionTooltip(
      ...
      localPanel.queryData.searchText
    );
  }
...
grid.on("change", function (event, items) {
  items.forEach(function (item) {
    ...
    let panelIndex = localPanels.findIndex(
      (panel) => panel.panelId === item.el.id
    );
    if (panelIndex !== -1) {
...
function displayPanels() {
  ...
  localPanels.forEach((localPanel) => {
    ...
    var newItem = grid.addWidget(
      `<div class="grid-stack-item" id="${idpanel}"><div class="grid-stack-item-content"></div></div>`,
      {
    ...
    $(
      `.grid-stack-item .grid-stack-item-content #panel${idpanel} .panel-header p`
    ).html(localPanel.name);

    if (
      localPanel.description ||
      (localPanel.queryData && localPanel.queryData.searchText)
    ) {
      handleDescriptionTooltip(
        idpanel,
        localPanel.description,
...
function addDuplicatePanel(panelToDuplicate) {
  ...
  var newItem = grid.addWidget(
    `<div class="grid-stack-item" id="${idpanel}"><div class="grid-stack-item-content"></div></div>`,
    { width: panelToDuplicate.gridpos.w, height: panelToDuplicate.gridpos.h }
  );
...
function addDefaultPanel() {
  var defaultItem = grid.addWidget(
    `<div class="grid-stack-item default-item active"><div class="add-panel-div">
    <div class="plus-icon">+</div>
    <div class="text">Select the Panel Type</div>
    </div></div>`,
    { width: 6, height: 3, noResize: true, noMove: true }
  );
