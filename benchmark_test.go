...
func Benchmark_RRCToJson(b *testing.B) {
	...
	currTime := segutils.GetCurrentTimeMillis()
	startTime := uint64(0)
	...
	colVal, err := segutils.CreateDtypeEnclosure("batch-101", 1)
...
func Benchmark_E2E_AgileTree(b *testing.B) {
	...
	colVal, err := segutils.CreateDtypeEnclosure("*", 1)
	if err != nil {
		log.Fatal(err)
	}
	valueFilter := structs.FilterCriteria{
...
func benchmarkBloom(strs [][]byte) {
	...
	bloom := bloom.NewWithEstimates(numItems, segutils.BLOOM_COLL_PROBABILITY)
