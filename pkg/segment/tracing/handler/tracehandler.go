...
func DependencyGraphThread() {
	for {
		...
		endEpoch := time.Now().UnixNano() / int64(time.Millisecond)
		startEpoch := time.Now().Add(-time.Hour).UnixNano() / int64(time.Millisecond)

		depMatrix := MakeTracesDependancyGraph(startEpoch, endEpoch)
		writeDependencyMatrix(depMatrix, true)
	}
}
...
func writeDependencyMatrix(dependencyMatrix map[string]map[string]int, isAutoGenerated bool) {
	if !isAutoGenerated {
		return
	}
...
func ProcessDepgraphRequest(ctx *fasthttp.RequestCtx, myid uint64) {
	// Extract startEpoch and endEpoch from the request
	_, readJSON, err := ParseAndValidateRequestBody(ctx)
	if err != nil {
