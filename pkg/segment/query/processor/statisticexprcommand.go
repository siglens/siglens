...
func (p *statisticExprProcessor) processAggregationResult(aggResults map[string]*structs.AggregationResult) (*iqr.IQR, error) {
	...
	countIsGroupByCol := toputils.SliceHas(statsFields, p.options.StatisticOptions.CountField)
	percentIsGroupByCol := toputils.SliceHas(statsFields, p.options.StatisticOptions.PercentField)
	...
	for _, aggregationResult := range aggResults {
		...
		for _, bucketResult := range aggregationResult.Results {
			...
			countIsStatisticGroupByCol := toputils.SliceHas(statsFields, countName)
