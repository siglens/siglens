...
func Test_InputLookup(t *testing.T) {
	...
	expected := []sutils.CValueEnclosure{
		{Dtype: sutils.SS_DT_STRING, CVal: "a4"},
		{Dtype: sutils.SS_DT_STRING, CVal: "a7"},
	}
	...
	assert.Nil(t, err)
	expected = []sutils.CValueEnclosure{
		{Dtype: sutils.SS_DT_STRING, CVal: "b5"},
		{Dtype: sutils.SS_DT_STRING, CVal: "b8"},
	}
	...
	assert.Nil(t, err)
	expected = []sutils.CValueEnclosure{
		{Dtype: sutils.SS_DT_STRING, CVal: "c6"},
		{Dtype: sutils.SS_DT_STRING, CVal: "c9"},
	}
...
func getExpectedColumnValues(val string, start int, count int) []sutils.CValueEnclosure {
	var expected []sutils.CValueEnclosure
	for i := 0; i < count; i++ {
		colValue := fmt.Sprintf("%v%v", val, start+i)
		expected = append(expected, sutils.CValueEnclosure{Dtype: sutils.SS_DT_STRING, CVal: colValue})
	}
	return expected
}
...
func Test_MultipleInputlookups(t *testing.T) {
	...
	assert.Nil(t, err)
	expected := []sutils.CValueEnclosure{
		{Dtype: sutils.SS_DT_STRING, CVal: "a1"},
		{Dtype: sutils.SS_DT_STRING, CVal: "a4"},
		{Dtype: sutils.SS_DT_FLOAT, CVal: float64(40)},
		{Dtype: sutils.SS_DT_FLOAT, CVal: float64(70)},
	}
	assert.Equal(t, expected, col_a)
	...
	assert.Nil(t, err)
	expected = []sutils.CValueEnclosure{
		{Dtype: sutils.SS_DT_STRING, CVal: "b2"},
		{Dtype: sutils.SS_DT_STRING, CVal: "b5"},
		{Dtype: sutils.SS_DT_FLOAT, CVal: float64(50)},
		{Dtype: sutils.SS_DT_FLOAT, CVal: float64(80)},
	}
	assert.Equal(t, expected, col_b)
	...
	assert.Nil(t, err)
	expected = []sutils.CValueEnclosure{
		{Dtype: sutils.SS_DT_STRING, CVal: "c3"},
		{Dtype: sutils.SS_DT_STRING, CVal: "c6"},
		{Dtype: sutils.SS_DT_FLOAT, CVal: float64(60)},
		{Dtype: sutils.SS_DT_FLOAT, CVal: float64(90)},
	}
	assert.Equal(t, expected, col_c)
