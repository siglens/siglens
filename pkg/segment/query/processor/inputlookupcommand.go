...
func createRecord(columnNames []string, record []string) (map[string]sutils.CValueEnclosure, error) {
	...
	recordMap := make(map[string]sutils.CValueEnclosure)
	for i, cname := range columnNames {
		floatVal, floatErr := strconv.ParseFloat(record[i], 64)
		if floatErr != nil {
			recordMap[cname] = sutils.CValueEnclosure{Dtype: sutils.SS_DT_STRING, CVal: record[i]}
		} else {
			recordMap[cname] = sutils.CValueEnclosure{Dtype: sutils.SS_DT_FLOAT, CVal: floatVal}
		}
	}
...
func (p *inputlookupProcessor) Process(inpIqr *iqr.IQR) (*iqr.IQR, error) {
	...
	if p.limit == 0 {
		p.limit = sutils.QUERY_EARLY_EXIT_LIMIT // TODO: Find a better way to deal with scroll and set limit for later inputlookups
	}
