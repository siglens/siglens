...
func addGenTimeEvent(values map[string][]sutils.CValueEnclosure, start time.Time, end time.Time) {
	values["starttime"] = append(values["starttime"], sutils.CValueEnclosure{
		Dtype: sutils.SS_DT_UNSIGNED_NUM,
		CVal:  uint64(start.UnixMilli()) / 1000,
	})

	values["endttime"] = append(values["endttime"], sutils.CValueEnclosure{
		Dtype: sutils.SS_DT_UNSIGNED_NUM,
		CVal:  uint64(end.UnixMilli()) / 1000,
	})

	values["starthuman"] = append(values["starthuman"], sutils.CValueEnclosure{
		Dtype: sutils.SS_DT_STRING,
		CVal:  utils.FormatToHumanReadableTime(start),
	})

	values["endhuman"] = append(values["endhuman"], sutils.CValueEnclosure{
		Dtype: sutils.SS_DT_STRING,
...
func (p *gentimesProcessor) Process(inpIqr *iqr.IQR) (*iqr.IQR, error) {
	...
	knownValues := make(map[string][]sutils.CValueEnclosure, 0)

	count := uint64(0)
	for curr < p.options.EndTime && count < p.limit {
		endTime, err := sutils.ApplyOffsetToTime(int64(p.options.Interval.Num), p.options.Interval.TimeScalr, currTime)
		if err != nil {
