...
func TestComputeAggEvalForList_InvalidRunningEvalStatsConversion(t *testing.T) {
	...
	measureResults := make(map[string]segutils.CValueEnclosure)
...
func TestComputeAggEvalForList_largeLists(t *testing.T) {
	...
	list := make([]*segutils.CValueEnclosure, segutils.MAX_SPL_LIST_SIZE+100)

	for i := range list {
		list[i] = &segutils.CValueEnclosure{
			CVal:  1.0,
			Dtype: segutils.SS_DT_FLOAT,
		}
	}
	...
	err := ComputeAggEvalForList(dummyMeasureAggr, sstMap, measureResults, runningEvalStats)
	assert.NoError(t, err)
	assert.Equal(t, segutils.MAX_SPL_LIST_SIZE, len(measureResults["vals"].CVal.([]string)), "The length of the measureResults slice should be MAX_SPL_LIST_SIZE")
	assert.Equal(t, segutils.MAX_SPL_LIST_SIZE, len(runningEvalStats["vals"].([]string)), "The length of the runningEvalStats slice should be MAX_SPL_LIST_SIZE")
}
...
func TestComputeAggEvalForValues_NoFields(t *testing.T) {
	...
	sstMap := map[string]*structs.SegStats{"vals": getDummySegStats()}
	measureResults := map[string]segutils.CValueEnclosure{}
	runningEvalStats := map[string]interface{}{
