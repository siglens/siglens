...
func Perc(values []float64, percentile float64) float64 {
	if len(values) == 0 {
		return math.NaN() // Return NaN for empty input
	}

	// Use 66.6 as the default percentile if an invalid value is provided
	...
	// Return the value at the rank index
...
func UpperPerc(values []float64, percentile float64) float64 {
	...
	// Sort only the upper portion of the data using a partial sort
	sort.Float64s(values)

	// Compute the rank index for the upper bound
	...
	// Return the estimated upper-bound percentile value
	return values[index]
}