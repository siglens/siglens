...
func PerformStreamStatsOnSingleFunc(currIndex int, bucketKey string, ssOption *structs.StreamStatsOptions, measureFuncIndex int,
	measureAgg *structs.MeasureAggregator, record map[string]interface{}, timestamp uint64, timeSortAsc bool) (sutils.CValueEnclosure, bool, error) {
	...
	if !exist && measureAgg.ValueColRequest == nil {
		return sutils.CValueEnclosure{}, false, fmt.Errorf("PerformStreamStatsOnSingleFunc: Error, measure column: %v not found in the record", measureAgg.MeasureCol)
	}
	...
	if measureAgg.ValueColRequest != nil {
		...
		if err != nil {
			return sutils.CValueEnclosure{}, false, fmt.Errorf("PerformStreamStatsOnSingleFunc: Error while fetching values, err: %v", err)
		}
	}
	...
	if ssOption.Window == 0 && ssOption.TimeWindow == nil {
		...
		if err != nil {
			return sutils.CValueEnclosure{}, false, fmt.Errorf("PerformStreamStatsOnSingleFunc: Error while performing global stream stats on function %v for value %v, err: %v", measureAgg.MeasureFunc, colValue, err)
		}
	} else {
		...
		if err != nil {
			return sutils.CValueEnclosure{}, false, fmt.Errorf("PerformStreamStatsOnSingleFunc: Error while performing window stream stats on function %v for value %v, err: %v", measureAgg.MeasureFunc, colValue, err)
		}
	}
...
func getRecordFromFieldToValue(fieldToValue map[string]sutils.CValueEnclosure) map[string]interface{} {
			...
			for _, measureAgg := range measureAggs {
				...
				if isGroupByCol {
					...
					if streamStatsResult != "" {
						dataType := sutils.SS_DT_FLOAT
						if measureAgg.MeasureFunc == sutils.Values {
							dataType = sutils.SS_DT_STRING_SLICE
						}
