...
func printMemoryManagerSummary() {
	...
	log.Infof("GlobalMemoryTracker: MetricsEncodedSizeInfo: TotalTagTrees: %d, TotalLeafNodes: %d, TotalTagsTreeSize: %.4f MB. TotalSeriesCount: %d, TotalTSIDs: %d, TotalTSIDLookup (reverse Index): %d, TotalMSegmentsEncodedSizeInBytes(AllBlock): %.4f MB, TotalInMemoryMSegEncodedSize: %.4f MB",
		...
		utils.ConvertFloatBytesToMB(float64(metricsSizeInfo.TotalInMemoryMSegEncodedSizeInBytes)))
