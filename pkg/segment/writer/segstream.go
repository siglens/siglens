...
func applyStreamingSearchToRecord(segStore *SegStore, psNode map[string]*structs.SearchNode,
	...
	holderDte := &segutils.DtypeEnclosure{}
...
func applySearchSingleNode(colWips map[string]*ColWip, sNode *structs.SearchNode, holderDte *segutils.DtypeEnclosure, tsKey string, segStore *SegStore) bool {
	retVal := false
	if sNode.AndSearchConditions != nil {
		andConditions := applySearchSingleCondition(colWips, sNode.AndSearchConditions, segutils.And, holderDte, tsKey, segStore)
		if !andConditions {
			return false
		}
		retVal = true
	...
	if sNode.OrSearchConditions != nil {
		orConditions := applySearchSingleCondition(colWips, sNode.OrSearchConditions, segutils.Or, holderDte, tsKey, segStore)
	...
	if sNode.ExclusionSearchConditions != nil {
		exclusionConditions := applySearchSingleCondition(colWips, sNode.ExclusionSearchConditions, segutils.Exclusion, holderDte, tsKey, segStore)
...
func applySearchSingleCondition(colWips map[string]*ColWip, sCond *structs.SearchCondition, op segutils.LogicalOperator,
	holderDte *segutils.DtypeEnclosure, tsKey string, segStore *SegStore) bool {
	orMatch := false
...
func applySearchSingleQuery(colWips map[string]*ColWip, sQuery *structs.SearchQuery, op segutils.LogicalOperator,
	holderDte *segutils.DtypeEnclosure, tsKey string, segStore *SegStore) bool {
