...
func TestStarTree(t *testing.T) {
	...
	for i, test := range cases {
		...
		_, err = ss.EncodeColumns(raw, uint64(i), &tsKey, sutils.SIGNAL_EVENTS,
			cnameCacheByteHashToStr, jsParsingStackbuf[:])
		assert.NoError(t, err)

		for _, cwip := range ss.wipBlock.colWips {
			ss.wipBlock.maxIdx = sutils.MaxUint32(ss.wipBlock.maxIdx, cwip.cbufidx)
		}
...
func populateAggsFromTree(t *testing.T, root *Node, aggValues map[string][]*sutils.Number, key string) {
	var newKey string
	if key == "" {
...
func testAggs(t *testing.T, expected []int, aggValues []*sutils.Number) {
...
func TestStarTree2(t *testing.T) {
	...
	aggValues := make(map[string][]*sutils.Number)
	...
	aggValues = make(map[string][]*sutils.Number, 0)
	populateAggsFromTree(t, root, aggValues, "")
