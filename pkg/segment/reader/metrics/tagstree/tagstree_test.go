...
func Test_ReadWriteTagsTree(t *testing.T) {
	...
	exists, tagValExists, rawTagValueToTSIDs, tagHashValue, err := attr.getOrInsertMatchingTSIDs(metricName, tagKey, tagValue, segutils.Equal, nil)
	assert.Nil(t, err)
...
func Test_SelectOneTagKeyValuePair(t *testing.T) {
	...
	exists, tagValExists, rawTagValueToTSIDs, _, err := attr.getOrInsertMatchingTSIDs(metric1, "color", xxhash.Sum64String("blue"), segutils.Equal, nil)
	assert.Nil(t, err)
	assert.True(t, exists)
	assert.True(t, tagValExists)
	...
	exists, tagValExists, rawTagValueToTSIDs, _, err = attr.getOrInsertMatchingTSIDs(metric1, "fruit", xxhash.Sum64String("pear"), segutils.Equal, nil)
	assert.Nil(t, err)
	assert.True(t, exists)
	assert.True(t, tagValExists)
	...
	// Test selecting for key != value
	exists, tagValExists, rawTagValueToTSIDs, _, err = attr.getOrInsertMatchingTSIDs(metric1, "color", xxhash.Sum64String("green"), segutils.NotEqual, nil)
	assert.Nil(t, err)
	assert.True(t, exists)
	assert.True(t, tagValExists)
	assert.Equal(t, numTSIDs(rawTagValueToTSIDs), 4)

	exists, tagValExists, rawTagValueToTSIDs, _, err = attr.getOrInsertMatchingTSIDs(metric1, "fruit", xxhash.Sum64String("pear"), segutils.NotEqual, nil)
	assert.Nil(t, err)
	assert.True(t, exists)
	assert.True(t, tagValExists)
	assert.Equal(t, numTSIDs(rawTagValueToTSIDs), 2)

	exists, tagValExists, rawTagValueToTSIDs, _, err = attr.getOrInsertMatchingTSIDs(metric1, "fruit", xxhash.Sum64String("this-doesn't-match-anything"), segutils.NotEqual, nil)
	assert.Nil(t, err)
	assert.True(t, exists)
	assert.True(t, tagValExists)
