...
func buildSegMapOldPipeline(allrrc []*sutils.RecordResultContainer, segEncToKey map[uint32]string) (map[string]*sutils.BlkRecIdxContainer, map[string]int) {
	segmap := make(map[string]*sutils.BlkRecIdxContainer)
	...
	for idx, rrc := range allrrc {
		...
		if !ok {
			...
			blkIdxsCtr = &sutils.BlkRecIdxContainer{BlkRecIndexes: innermap, VirtualTableName: rrc.VirtualTableName}
...
func GetJsonFromAllRrcOldPipeline(allrrc []*sutils.RecordResultContainer, esResponse bool, qid uint64,
...
func extractKeyValuePairsFromString(str string) (map[string]interface{}, error) {
	...
	for _, pair := range pairs {
		...
		if len(parts) == 2 {
			...
			keyValuePairs[key] = sutils.GetLiteralFromString(value)
		} else {
