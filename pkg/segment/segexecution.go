...
func ExecuteMultipleMetricsQuery(hashList []uint64, mQueries []*structs.MetricsQuery, queryOps []structs.QueryArithmetic, timeRange *dtu.MetricsTimeRange, qid uint64, timeseriesApi bool) *mresults.MetricsResult {
	...
	return ProcessQueryArithmeticAndLogical(queryOps, resMap, timeseriesApi)
}

func ProcessQueryArithmeticAndLogical(queryOps []structs.QueryArithmetic, resMap map[uint64]*mresults.MetricsResult, timeseriesApi bool) *mresults.MetricsResult {
	...
	finalResult, err := processQueryArithmeticNodeOp(&queryOps[0], resMap, &operationCounter, timeseriesApi)
	if err != nil {
...
func processQueryArithmeticNodeOp(queryOp *structs.QueryArithmetic, resMap map[uint64]*mresults.MetricsResult, operationCounter *int, timeseriesApi bool) (map[string]map[uint32]float64, error) {
	if queryOp == nil {
	...
	if queryOp.LHSExpr != nil {
		...
		result, err := processQueryArithmeticNodeOp(queryOp.LHSExpr, resMap, operationCounter, timeseriesApi)
		if err != nil {
			return nil, err
		}
		if len(result) > 0 {
	...
	if queryOp.RHSExpr != nil {
		...
		result, err := processQueryArithmeticNodeOp(queryOp.RHSExpr, resMap, operationCounter, timeseriesApi)
		if err != nil {
			return nil, err
		}
		if len(result) > 0 {
	...
	*operationCounter++
	...
	return HelperQueryArithmeticAndLogical(queryOp, resMap, timeseriesApi)
}

func HelperQueryArithmeticAndLogical(queryOp *structs.QueryArithmetic, resMap map[uint64]*mresults.MetricsResult, timeseriesApi bool) (map[string]map[uint32]float64, error) {
	...
	if queryOp.ConstantOp {
		...
		for lGroupID, tsLHS := range resultLHS.Results {
			...
			if hasVectorMatchingOp || timeseriesApi {
