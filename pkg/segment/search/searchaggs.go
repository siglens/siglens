...
func applyAggregationsToSingleBlock(multiReader *segread.MultiColSegmentReader, aggs *structs.QueryAggregators,
	...
	for blockStatus := range blockChan {
		...
		recIT, err := blockStatus.GetRecordIteratorCopyForBlock(segutils.And)
		if err != nil {
...
func addRecordToAggregations(grpReq *structs.GroupByRequest, timeHistogram *structs.TimeBucket, measureInfo map[string][]int, numMFuncs int,
	...
	measureResults := make([]segutils.CValueEnclosure, numMFuncs)
	var retCVal segutils.CValueEnclosure
	...
	unsetRecord := make(map[string]segutils.CValueEnclosure) // Make this once instead of each iteration.
	...
	for recNum := uint16(0); recNum < recIT.AllRecLen; recNum++ {
		...
		if usedByTimechart {
			...
			copy(aggsKeyWorkingBuf[aggsKeyBufIdx:], segutils.VALTYPE_ENC_UINT64[:])
			aggsKeyBufIdx += 1
			...
			if byFieldCnameKeyIdx != -1 {
				...
				if err != nil {
					...
					if !exists {
						strs, err = segutils.ConvertGroupByKey(rawVal)
						if err != nil {
			...
			// column's record
			if len(aggsKeyWorkingBuf) < len(groupbyColKeyIndices)*segutils.MAX_RECORD_SIZE {
				aggsKeyWorkingBuf = utils.ResizeSlice(aggsKeyWorkingBuf,
					len(groupbyColKeyIndices)*segutils.MAX_RECORD_SIZE)
			}
			for _, colKeyIndex := range groupbyColKeyIndices {
				...
				if err != nil {
					...
					copy(aggsKeyWorkingBuf[aggsKeyBufIdx:], segutils.VALTYPE_ENC_BACKFILL)
					aggsKeyBufIdx += 1
				} else {
...
func PerformGroupByRequestAggsOnRecs(nodeResult *structs.NodeResult, recs map[string]map[string]interface{}, finalCols map[string]bool, qid uint64, numTotalSegments uint64, sizeLimit uint64) map[string]bool {
	...
	if nodeResult.RecsAggregator.GroupByRequest != nil && nodeResult.RecsAggregator.GroupByRequest.MeasureOperations != nil {
		for _, mOp := range nodeResult.RecsAggregator.GroupByRequest.MeasureOperations {
			if mOp.MeasureFunc == segutils.Count {
	...
	for recInden, record := range recs {
		...
		for cname, indices := range measureInfo {
			...
			if !exists {
				...
				cVal = segutils.CValueEnclosure{Dtype: segutils.SS_DT_BACKFILL}
			} else {
				dval, err := segutils.CreateDtypeEnclosure(value, qid)
				if dval.Dtype == segutils.SS_DT_STRING {
					floatFieldVal, _ := dtu.ConvertToFloat(value, 64)
					if err == nil {
						value = floatFieldVal
						dval.Dtype = segutils.SS_DT_FLOAT
					}
				}
				...
				if err != nil {
					...
					cVal = segutils.CValueEnclosure{Dtype: dval.Dtype, CVal: value}
				}
			}
	...
	for bKey, index := range blockRes.GroupByAggregation.StringBucketIdx {
		...
		for i, mOp := range internalMops {
			...
			if mOp.MeasureFunc == segutils.Count {
				...
				if mOp.OverrodeMeasureAgg != nil && mOp.OverrodeMeasureAgg.MeasureFunc == segutils.Avg {
...
func PerformMeasureAggsOnRecs(nodeResult *structs.NodeResult, recs map[string]map[string]interface{}, finalCols map[string]bool, qid uint64, numTotalSegments uint64, sizeLimit uint64) map[string]bool {
	...
	for recInden, record := range recs {
		...
		for _, mOp := range nodeResult.RecsAggregator.MeasureOperations {
			dtypeVal, err := segutils.CreateDtypeEnclosure(record[mOp.MeasureCol], qid)
			if err != nil {
			...
			if segutils.IsNumTypeAgg(mOp.MeasureFunc) {
				if !dtypeVal.IsNumeric() {
					...
					dtypeVal = &segutils.DtypeEnclosure{Dtype: segutils.SS_DT_FLOAT, FloatVal: floatVal}
					segStat.IsNumeric = true
				}

				// Populate numeric stats if dtypeVal holds a numeric type now.
				if dtypeVal.IsNumeric() {
					nTypeEnclosure := &segutils.NumTypeEnclosure{
			...
			} else if mOp.MeasureFunc != segutils.Count {
				...
				if dtypeVal.Dtype == segutils.SS_DT_STRING_SLICE {
		...
		for colName, value := range searchResults.GetSegmentStatsMeasureResults() {
			...
			case segutils.SS_DT_STRING_SLICE:
				...
				if err != nil {
					...
					value.Dtype = segutils.SS_INVALID
				} else {
					value.CVal = strVal
				}
			...
			if value.Dtype != segutils.SS_INVALID {
...
func GetAggColsAndTimestamp(aggs *structs.QueryAggregators) (map[string]bool, map[string]segutils.AggColUsageMode, map[string]bool) {
	...
	aggColUsage := make(map[string]segutils.AggColUsageMode)
...
func GetSegStatsMeasureCols(ops []*structs.MeasureAggregator) (map[string]bool, map[string]segutils.AggColUsageMode, map[string]bool, map[string]bool) {
	// Determine if current col used by eval statements
	aggColUsage := make(map[string]segutils.AggColUsageMode)
	// Determine if current col used by agg values() func
	valuesUsage := make(map[string]bool)
...
func segmentStatsWorker(statRes *segresults.StatsResults, mCols map[string]bool, aggColUsage map[string]segutils.AggColUsageMode, valuesUsage map[string]bool, listUsage map[string]bool,
	...
	for blockStatus := range blockChan {
		isBlkFullyEncosed := queryRange.AreTimesFullyEnclosed(blockSummaries[blockStatus.BlockNum].LowTs,
			blockSummaries[blockStatus.BlockNum].HighTs)
		recIT, err := blockStatus.GetRecordIteratorForBlock(segutils.And)
		if err != nil {
		...
		for _, recNum := range sortedMatchedRecs {
			for colKeyIdx, cname := range nonDeColsKeyIndices {
				...
				if cValEnc.Dtype == segutils.SS_DT_BACKFILL {
					continue
				}
				...
				if cValEnc.Dtype == segutils.SS_DT_STRING {
					...
					var intVal int64
					var valueType segutils.SS_IntUintFloatTypes
					var numStr string
					var err error
					if cValEnc.IsFloat() {
						valueType = segutils.SS_FLOAT64
						floatVal, err = cValEnc.GetFloatValue()
						numStr = fmt.Sprintf("%v", floatVal)
					} else {
						valueType = segutils.SS_INT64
...
func applySegmentStatsUsingDictEncoding(mcr *segread.MultiColSegmentReader, filterdRecNums []uint16, mCols map[string]bool, aggColUsage map[string]segutils.AggColUsageMode, valuesUsage map[string]bool, listUsage map[string]bool,
	...
	for colName := range mCols {
		...
		for _, cMap := range results {
			for colName, rawVal := range cMap {
				...
				if !exists {
					colUsage = segutils.NoEvalUsage
				}
				// If current col will be used by eval funcs, we should store the raw data and process it
				if colUsage == segutils.WithEvalUsage || colUsage == segutils.BothUsage {
					e := segutils.CValueEnclosure{}
					...
					if e.Dtype != segutils.SS_DT_STRING {
						retVal[colName] = true
						continue
					}
					...
					if !ok {
						stats = &structs.SegStats{
							IsNumeric: false,
							Count:     0,
							Records:   make([]*segutils.CValueEnclosure, 0),
						}
						stats.CreateNewHll()
				...
				case int64:
					stats.AddSegStatsNums(lStats, colName, segutils.SS_INT64, val, 0, 0, fmt.Sprintf("%v", val), bb, aggColUsage, hasValuesFunc, hasListFunc)
				case float64:
					stats.AddSegStatsNums(lStats, colName, segutils.SS_FLOAT64, 0, 0, val, fmt.Sprintf("%v", val), bb, aggColUsage, hasValuesFunc, hasListFunc)
				default:
...
func iterRecsAddRrc(recIT *BlockRecordIterator, mcr *segread.MultiColSegmentReader,
	...
	for recNum := uint(0); recNum < uint(recIT.AllRecLen); recNum++ {
		...
		if config.IsNewQueryPipelineEnabled() {
			rrc := &segutils.RecordResultContainer{
