...
func GetSegStatsMeasureCols(ops []*structs.MeasureAggregator) (map[string]bool, map[string]sutils.AggColUsageMode, map[string]bool, map[string]bool) {
	// Determine if current col used by eval statements
	aggColUsage := make(map[string]sutils.AggColUsageMode)
	// Determine if current col used by agg values() func
	valuesUsage := make(map[string]bool)
...
func segmentStatsWorker(statRes *segresults.StatsResults, mCols map[string]bool, aggColUsage map[string]sutils.AggColUsageMode, valuesUsage map[string]bool, listUsage map[string]bool,
	...
	for blockStatus := range blockChan {
		...
		nonDeCols := applySegmentStatsUsingDictEncoding(multiReader, sortedMatchedRecs, mCols, aggColUsage, valuesUsage, listUsage, blockStatus.BlockNum, recIT, localStats, bb, qid, latestTs, earliestTs, needLatestOrEarliest)
		...
		for _, recNum := range sortedMatchedRecs {
			for colKeyIdx, cname := range nonDeColsKeyIndices {
				...
				stats.AddSegStatsUNIXTime(localStats, cname, earliestTs, cValEnc.CVal, false)
				if needLatestOrEarliest {
					tsCVal := sutils.CValueEnclosure{}
					timestampIdx := -1
					err := multiReader.ExtractValueFromColumnFile(timestampIdx, blockStatus.BlockNum, recNum, qid, true, &tsCVal)
					if err != nil {
						log.Errorf("qid=%d, segmentStatsWorker failed to get timestamp values for dict encoded column; col: %v", qid, cname)
					} else {
						stats.AddSegStatsLatestEarliestVal(localStats, cname, &tsCVal, cValEnc.CVal, true)
						stats.AddSegStatsLatestEarliestVal(localStats, cname, &tsCVal, cValEnc.CVal, false)
					}
				}
	...
	for colName := range mCols {
		...
		for recNum, cMap := range results {
			for colName, rawVal := range cMap {
				...
				stats.AddSegStatsUNIXTime(lStats, colName, earliestTs, rawVal, false)
				if needLatestOrEarliest {
					tsCVal := sutils.CValueEnclosure{}
					timestampIdx := -1
					...
					if err != nil {
						...
						stats.AddSegStatsLatestEarliestVal(lStats, colName, &tsCVal, rawVal, true)
						stats.AddSegStatsLatestEarliestVal(lStats, colName, &tsCVal, rawVal, false)
					}
				}
