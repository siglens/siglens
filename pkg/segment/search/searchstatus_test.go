...
func Test_InitSearchStatus(t *testing.T) {
	...
	for _, blkStatus := range status.AllBlockStatus {
		recITerator, err := blkStatus.GetRecordIteratorForBlock(segutils.And)
		assert.Nil(t, err)
...
func Test_FirstSearchRecords(t *testing.T) {
	...
	for blkNum := uint16(0); blkNum < numBlocks; blkNum++ {
		recITerator, err := status.GetRecordIteratorForBlock(segutils.And, blkNum)
		assert.Nil(t, err)
		assert.Equal(t, recITerator.AllRecLen, numRecs)
		for j := uint(0); j < uint(numRecs); j++ {
		...
		recITerator, err = status.GetRecordIteratorForBlock(segutils.Or, blkNum)
		assert.Nil(t, err)
		assert.Equal(t, recITerator.AllRecLen, numRecs)
		for j := uint(0); j < uint(numRecs); j++ {
			readNum := recITerator.ShouldProcessRecord(j)
		...
		recITerator, err = status.GetRecordIteratorForBlock(segutils.Exclusion, blkNum)
		assert.Nil(t, err)
		assert.Equal(t, recITerator.AllRecLen, numRecs)
		for j := uint(0); j < uint(numRecs); j++ {
			readNum := recITerator.ShouldProcessRecord(j)
...
func Test_UpdateAndSearch(t *testing.T) {
	...
	err := status.updateMatchedRecords(0, matched, segutils.And)
	assert.Nil(t, err)
	recITerator, err := status.GetRecordIteratorForBlock(segutils.And, 0)
	assert.Nil(t, err)
...
func Test_UpdateOrSearch(t *testing.T) {
	...
	matched := pqmr.CreatePQMatchResults(1)
	matched.AddMatchedRecord(0)
	err := status.updateMatchedRecords(0, matched, segutils.Or)
	assert.Nil(t, err)

	recITerator, err := status.GetRecordIteratorForBlock(segutils.Or, 0)
	assert.Nil(t, err)
	...
	err = status.updateMatchedRecords(0, moreOrMatch, segutils.Or)
	assert.Nil(t, err)
	recITerator, err = status.GetRecordIteratorForBlock(segutils.Or, 0)
	assert.Nil(t, err)
	assert.False(t, recITerator.firstSearch)
...
func Test_UpdateExclusionSearch(t *testing.T) {
	...
	matched := pqmr.CreatePQMatchResults(1)
	matched.AddMatchedRecord(0)
	err := status.updateMatchedRecords(0, matched, segutils.Exclusion)
	assert.Nil(t, err)

	recITerator, err := status.GetRecordIteratorForBlock(segutils.Exclusion, 0)
	assert.Nil(t, err)
	assert.False(t, recITerator.firstSearch)
	orCount := 0
	startTs := uint64(1)
