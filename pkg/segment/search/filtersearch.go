...
func logSingleQuerySummary(segmentSearch *SegmentSearchStatus, op segutils.LogicalOperator, qid uint64) {
	if config.IsDebugMode() {
		opStr := segutils.ConvertOperatorToString(op)
...
func filterBlockRequestFromQuery(multiColReader *segread.MultiColSegmentReader, query *structs.SearchQuery,
	...
	runningBlockManagers *sync.WaitGroup, op segutils.LogicalOperator, queryType structs.SearchNodeType,
...
func filterRecordsFromSearchQuery(query *structs.SearchQuery, segmentSearch *SegmentSearchStatus,
	...
	blockNum uint16, holderDte *segutils.DtypeEnclosure, qid uint64, allSearchResults *segresults.SearchResults,
