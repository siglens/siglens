...
func getStats(myid int64, filterFunc func(string) bool, allSegMetas []*structs.SegMeta) (utils.AllIndexesStats, int64, float64, float64, map[string]struct{}) {
	...
	for _, indexName := range indices {
		...
		indexSegStats, err := writer.GetIndexSizeStats(indexName, utils.Some(myid))
		if err != nil {
		...
		unrotatedByteCount, unrotatedEventCount, unrotatedOnDiskBytesCount, columnNamesSet := segwriter.GetUnrotatedVTableCounts(indexName, utils.Some(myid))
