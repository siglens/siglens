...
func Test_parsePromQLQuery_simpleQueries(t *testing.T) {
	...
	assert.Equal(t, sutils.Avg, mQueryReqs[0].MetricsQuery.SubsequentAggs.AggregatorBlock.AggregatorFunction)
	...
	assert.Equal(t, sutils.Rate, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.FunctionBlock.RangeFunction)
	...
	assert.False(t, mQueryReqs[0].MetricsQuery.Groupby)
	assert.Equal(t, intervalSeconds, mQueryReqs[0].MetricsQuery.Downsampler.Interval)
	assert.Equal(t, sutils.Sum, mQueryReqs[0].MetricsQuery.SubsequentAggs.AggregatorBlock.AggregatorFunction)
	assert.Nil(t, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next)
	...
	assert.Equal(t, sutils.Avg, mQueryReqs[0].MetricsQuery.SubsequentAggs.AggregatorBlock.AggregatorFunction)
	assert.NotNil(t, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next)
	assert.Equal(t, sutils.Avg_Over_Time, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.FunctionBlock.RangeFunction)
	assert.Equal(t, float64(300), mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.FunctionBlock.TimeWindow)
	...
	assert.Equal(t, sutils.Avg, mQueryReqs[0].MetricsQuery.SubsequentAggs.AggregatorBlock.AggregatorFunction)
	assert.NotNil(t, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next)
	assert.Equal(t, sutils.Clamp, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.FunctionBlock.MathFunction)
	...
	assert.Equal(t, sutils.Avg, mQueryReqs[0].MetricsQuery.SubsequentAggs.AggregatorBlock.AggregatorFunction)
	assert.NotNil(t, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next)
	assert.Equal(t, sutils.Clamp_Max, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.FunctionBlock.MathFunction)
	assert.Equal(t, functionValues, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.FunctionBlock.ValueList)
	...
	assert.Equal(t, sutils.Rate, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.FunctionBlock.RangeFunction)
	assert.Equal(t, float64(300), mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.FunctionBlock.TimeWindow)
	assert.Equal(t, sutils.LetMultiply, queryArithmetic[0].Operation)
	...
	assert.Equal(t, sutils.Avg, mQueryReqs[1].MetricsQuery.SubsequentAggs.AggregatorBlock.AggregatorFunction)
	assert.NotNil(t, mQueryReqs[1].MetricsQuery.SubsequentAggs.Next)
	assert.Equal(t, sutils.Rate, mQueryReqs[1].MetricsQuery.SubsequentAggs.Next.FunctionBlock.RangeFunction)
	assert.Equal(t, float64(300), mQueryReqs[1].MetricsQuery.SubsequentAggs.Next.FunctionBlock.TimeWindow)
	assert.Equal(t, sutils.LetAdd, queryArithmetic[0].Operation)
	assert.NotNil(t, queryArithmetic[0].RHSExpr)
	assert.Equal(t, float64(2), queryArithmetic[0].RHSExpr.Constant)
	assert.Equal(t, sutils.LetDivide, queryArithmetic[0].RHSExpr.Operation)
}
...
func Test_parsePromQLQuery_SimpleQueries_v2(t *testing.T) {
	...
	assert.Equal(t, sutils.Avg, mQueryReqs[0].MetricsQuery.SubsequentAggs.AggregatorBlock.AggregatorFunction)
	assert.Nil(t, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next)
	assert.Equal(t, sutils.Round, queryArithmetic[0].MQueryAggsChain.FunctionBlock.MathFunction)
...
func Test_parsePromQLQuery_NestedQueries_v1(t *testing.T) {
	...
	assert.Equal(t, sutils.Sum, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next.AggregatorBlock.AggregatorFunction)
	...
	assert.NotNil(t, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next)
	assert.Equal(t, structs.FunctionBlock, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.AggBlockType)
	assert.Equal(t, sutils.Max_Over_Time, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.FunctionBlock.RangeFunction)
	assert.Equal(t, float64(3600), mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.FunctionBlock.TimeWindow)
	assert.NotNil(t, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next)
	assert.Equal(t, structs.AggregatorBlock, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next.AggBlockType)
	assert.Equal(t, sutils.Max, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next.AggregatorBlock.AggregatorFunction)
...
func Test_parsePromQLQuery_NestedQueries_v2(t *testing.T) {
	...
	assert.Equal(t, sutils.Clamp_Max, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next.Next.FunctionBlock.MathFunction)
	...
	assert.NotNil(t, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next.Next)
	assert.Equal(t, structs.FunctionBlock, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next.Next.AggBlockType)
	assert.Equal(t, sutils.Max_Over_Time, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next.Next.FunctionBlock.RangeFunction)
...
func Test_parsePromQLQuery_NestedQueries_v3(t *testing.T) {
	...
	assert.Equal(t, sutils.IRate, mQueryReqs[1].MetricsQuery.SubsequentAggs.Next.FunctionBlock.RangeFunction)
	assert.Equal(t, float64(300), mQueryReqs[1].MetricsQuery.SubsequentAggs.Next.FunctionBlock.TimeWindow)
	...
	assert.Equal(t, sutils.Avg, mQueryReqs[1].MetricsQuery.SubsequentAggs.Next.Next.AggregatorBlock.AggregatorFunction)
...
func Test_parsePromQLQuery_NestedQueries_v4(t *testing.T) {
	...
	assert.NotNil(t, mQueryReqs[1].MetricsQuery.SubsequentAggs.Next)
	assert.Equal(t, structs.FunctionBlock, mQueryReqs[1].MetricsQuery.SubsequentAggs.Next.AggBlockType)
	assert.Equal(t, sutils.Count_Over_Time, mQueryReqs[1].MetricsQuery.SubsequentAggs.Next.FunctionBlock.RangeFunction)
	assert.Equal(t, float64(300), mQueryReqs[1].MetricsQuery.SubsequentAggs.Next.FunctionBlock.TimeWindow)

	assert.Equal(t, structs.AggregatorBlock, mQueryReqs[2].MetricsQuery.SubsequentAggs.AggBlockType)
	assert.Equal(t, sutils.Avg, mQueryReqs[2].MetricsQuery.SubsequentAggs.AggregatorBlock.AggregatorFunction)
	...
	assert.Equal(t, sutils.Rate, mQueryReqs[2].MetricsQuery.SubsequentAggs.Next.FunctionBlock.RangeFunction)
	...
	assert.Equal(t, sutils.Sum, mQueryReqs[2].MetricsQuery.SubsequentAggs.Next.Next.AggregatorBlock.AggregatorFunction)
	...
	assert.Equal(t, sutils.LetDivide, queryArithmetic[0].LHSExpr.Operation)
...
func Test_parsePromQLQuery_NestedQueries_v5(t *testing.T) {
	...
	assert.NotNil(t, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next.Next)
	assert.Equal(t, structs.FunctionBlock, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next.Next.AggBlockType)
	assert.Equal(t, sutils.Avg_Over_Time, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next.Next.FunctionBlock.RangeFunction)
	assert.Equal(t, float64(600), mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next.Next.FunctionBlock.TimeWindow)
	...
	assert.Equal(t, sutils.Sum_Over_Time, mQueryReqs[1].MetricsQuery.SubsequentAggs.Next.Next.FunctionBlock.RangeFunction)
...
func Test_parsePromQLQuery_NestedQueries_v6(t *testing.T) {
	...
	assert.Equal(t, sutils.Abs, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next.FunctionBlock.MathFunction)
}
...
func Test_parsePromQLQuery_NestedQueries_v7(t *testing.T) {
	...
	assert.NotNil(t, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next)
	assert.Equal(t, structs.FunctionBlock, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next.AggBlockType)
	assert.Equal(t, sutils.Derivative, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next.FunctionBlock.RangeFunction)
...
func Test_parsePromQLQuery_NestedQueries_v8(t *testing.T) {
	...
	assert.Equal(t, sutils.LetSubtract, queryArithmetic[0].Operation)
	...
	assert.Equal(t, sutils.Sum, queryArithmetic[0].MQueryAggsChain.AggregatorBlock.AggregatorFunction)
...
func Test_parsePromQLQuery_NestedQueries_NestedGroupBy_v1(t *testing.T) {
	...
	assert.Equal(t, sutils.Max, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next.Next.AggregatorBlock.AggregatorFunction)
...
func Test_parsePromQLQuery_NestedQueries_NestedGroupBy_v2(t *testing.T) {
	...
	assert.Equal(t, sutils.Sum, queryArithmetic[0].MQueryAggsChain.Next.AggregatorBlock.AggregatorFunction)
...
func Test_parsePromQLQuery_Scalar_Op_v1(t *testing.T) {
	...
	assert.Equal(t, sutils.LetAdd, queryOp.Operation)
	assert.Equal(t, float64(0), queryOp.Constant)

	assert.Equal(t, sutils.LetAdd, queryOp.LHSExpr.Operation)
	...
	assert.Equal(t, sutils.LetMultiply, queryOp.RHSExpr.Operation)
...
func Test_parsePromQLQuery_HistogramQunatile(t *testing.T) {
	...
	assert.NotNil(t, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next)
	assert.Equal(t, structs.FunctionBlock, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next.AggBlockType)
	assert.Equal(t, sutils.HistogramQuantile, mQueryReqs[0].MetricsQuery.SubsequentAggs.Next.Next.FunctionBlock.HistogramFunction.Function)
...
func Test_parsePromQLQuery_Binary_With_Nested_Queries(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		assert.Equal(t, sutils.Avg, mQueryReqs[i].MetricsQuery.SubsequentAggs.AggregatorBlock.AggregatorFunction)
		...
		assert.Equal(t, sutils.LabelReplace, mQueryReqs[i].MetricsQuery.SubsequentAggs.Next.FunctionBlock.LabelFunction.FunctionType)
		...
		assert.Equal(t, sutils.LabelReplace, mQueryReqs[i].MetricsQuery.SubsequentAggs.Next.Next.FunctionBlock.LabelFunction.FunctionType)
	...
	assert.Equal(t, sutils.Avg, mQueryReqs[3].MetricsQuery.SubsequentAggs.AggregatorBlock.AggregatorFunction)
	...
	assert.Equal(t, sutils.LetOr, binaryOperation.Operation)
	...
	assert.Equal(t, sutils.LetSubtract, binaryOperation.RHSExpr.Operation)
	assert.Equal(t, float64(1), binaryOperation.RHSExpr.Constant)
	assert.NotNil(t, binaryOperation.RHSExpr.LHSExpr)
	assert.Equal(t, sutils.LetEquals, binaryOperation.RHSExpr.LHSExpr.Operation)
	...
	assert.Equal(t, sutils.Group, binaryOperation.RHSExpr.LHSExpr.MQueryAggsChain.AggregatorBlock.AggregatorFunction)
	...
	assert.Equal(t, sutils.Last_Over_Time, binaryOperation.RHSExpr.LHSExpr.MQueryAggsChain.Next.FunctionBlock.RangeFunction)
	...
	assert.Equal(t, sutils.LetOr, binaryOperation.LHSExpr.Operation)
	assert.Equal(t, sutils.LetOr, binaryOperation.LHSExpr.LHSExpr.Operation)
	...
	assert.Equal(t, sutils.Group, binaryOperation.LHSExpr.MQueryAggsChain.AggregatorBlock.AggregatorFunction)
	...
	assert.Equal(t, sutils.Sum, binaryOperation.LHSExpr.MQueryAggsChain.Next.AggregatorBlock.AggregatorFunction)
