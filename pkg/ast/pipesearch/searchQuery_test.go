...
func TestAST_simpleEqual(t *testing.T) {
	...
	res, _, _, err := parsePipeSearch(string(json_body), "Pipe QL", 1)
	assert.Nil(t, err)
...
func TestAST_simpleAnd_SegLevelStats(t *testing.T) {
	...
	assert.Equal(t, aggNode.MeasureOperations[0].MeasureFunc, sutils.Min)
}
...
func TestAST_blankSearchFilter_SegLevelStats(t *testing.T) {
	...
	assert.Equal(t, aggNode.MeasureOperations[0].MeasureFunc, sutils.Max)
}
...
func TestAST_simpleAnd_LetAgg_Single(t *testing.T) {
	...
	assert.Equal(t, aggNode.OutputTransforms.LetColumns.SingleColRequest.Oper, sutils.LogicalAndArithmeticOperator(LetGreaterThanOrEqualTo))
...
func TestAST_simpleAnd_seglevelStats_commaSeparated(t *testing.T) {
	...
	assert.Equal(t, aggNode.MeasureOperations[1].MeasureFunc, sutils.Max)
}
...
func TestAST_GroupByseglevelStats_commaSeparated(t *testing.T) {
	...
	assert.Equal(t, aggNode.GroupByRequest.MeasureOperations[0].MeasureFunc, sutils.Min)
	assert.Equal(t, aggNode.GroupByRequest.MeasureOperations[1].MeasureCol, "latency")
	assert.Equal(t, aggNode.GroupByRequest.MeasureOperations[1].MeasureFunc, sutils.Max)
