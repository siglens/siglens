...
func ProcessPipeSearchWebsocket(conn *websocket.Conn, orgid uint64, ctx *fasthttp.RequestCtx) {
	...
	searchText, startEpoch, endEpoch, sizeLimit, indexNameIn, scrollFrom, searchErr := ParseSearchBody(event, nowTs)
	if searchErr != nil {
		log.Errorf("qid=%d, ProcessPipeSearchWebsocket: parseSearchBody returning an error %+v!", qid, searchErr)
		wErr := conn.WriteJSON(createErrorResponse(searchErr.Error()))
		if wErr != nil {
			log.Errorf("qid=%d, ProcessPipeSearchWebsocket: failed to write error response to websocket! %+v", qid, wErr)
		}
		return
